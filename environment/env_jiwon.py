...
class PMSP:
    ...
    def _modeling(self):
        ...
        for j in range(self.num_job):
            ...
            processing_time = round(np.random.uniform(10, 20),3) if self.test_sample is None else self.test_sample['processing_time'][j]
    ...
    def reset(self):
        ...
        if self.is_train:
            self.pt_var = round(np.random.uniform(low=0.1, high=0.5),3)
            self.ddt = round(np.random.uniform(low=0.8, high=1.2),3)
...
if __name__ == '__main__':
    pmsp = PMSP(10, 5, 1, None,
                [0.5, 0.5], {"ATCS": [2.730, 1.153], "COVERT": 6.8})
    print('PMSP environment Ready!')
