...
def get_sample(num_job, test_num):
    ...
    for i in range(test_num):
        sample_dict[i] = dict()
        sample_dict[i]['processing_time'] = round(list(np.random.uniform(10, 20, size=num_job)),4)
...
if __name__ == "__main__":
    ...
    for num_job in [10]:
        ...
        for i in range(5):
            sample_dict[i] = dict()
            sample_dict[i]['processing_time'] = [round(x, 3) for x in np.random.uniform(10, 20, size=num_job)]
            sample_dict[i]['feature'] = np.random.randint(0, 6, size=num_job)
